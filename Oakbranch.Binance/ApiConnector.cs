    ...
    public sealed class ApiConnector : IApiConnector, IDisposable
    {
        ...
        public async Task<Response> SendAsync(QueryParams query, CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (query.IsSecured) CompleteAndSign(ref queryString);
                fullEndpoint = CreateFullEndpoint(in query.BaseEndpoint, in query.RelativeEndpoint, queryString);    
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        PostLogMessage(LogLevel.Error, $"Failed to extract the rate limits usage info from the HTTP headers:\r\n{exc}");
                    }
            ...
            catch (WebException wExc)
            {
                ...
                throw GenerateQueryException((int)((HttpWebResponse)wExc.Response).StatusCode, wExc.Message);
            }
            catch (Exception exc)
            {
        ...
        private string CreateFullEndpoint(in string baseEndpoint, in string relativeEndpoint, QueryBuilder queryString)
        {
            if (queryString != null)
            {
        ...
        private void CompleteAndSign(ref QueryBuilder rawQuery)
        {
            ...
            if (rawQuery == null) rawQuery = new QueryBuilder(123);
        ...
        private long? DetermineRetryAfterTime(HttpStatusCode statusCode, HttpResponseHeaders headers)
        {
            ...
            if (retryValue != null)
            {
                ...
                catch (Exception exc)
                {
                    PostLogMessage(
                        LogLevel.Error,
                        $"The retry after time cannot be retrieved from HTTP response headers. The default delay will be returned.\r\n{exc}");
