...
namespace Oakbranch.Binance.Spot
{
    public class SpotAccountApiClient : ApiV3ClientBase
    {
        ...
        /// <param name="limit">The maximum number of the items to fetch. The maximum is 1000, the default is 500.</param>
        /// <returns></returns>
        public IDeferredQuery<List<SpotOrder>> PrepareGetOrders(string symbol, DateTime? startTime, DateTime? endTime, int? limit)
        {
            return PrepareGetOrders(
                symbol: symbol,
                fromId: null,
        ...
        /// </summary>
        public Task<List<SpotOrder>> GetOrdersAsync(string symbol, DateTime? startTime, DateTime? endTime, int? limit, CancellationToken ct = default)
        {
        ...
        // Orders parse logic.
        public List<SpotOrder> ParseOrderList(byte[] data, object parseArgs)
        {
        ...
        public SpotOrder ParseOrder(byte[] data, object parseArgs)
        {
            Utf8JsonReader reader = new Utf8JsonReader(data, ParseUtility.ReaderOptions);
            ParseUtility.ReadObjectStart(ref reader);
        ...
        private SpotOrder ParseOrder(ref Utf8JsonReader reader)
        {
            ...
            if (!validator.IsComplete())
            {
                ...
                switch (missingPropNum)
                {
                    case 0: throw ParseUtility.GenerateMissingPropertyException(objName, "symbol");
                    case 1: throw ParseUtility.GenerateMissingPropertyException(objName, "order ID");
                    case 2: throw ParseUtility.GenerateMissingPropertyException(objName, "order status");
                    case 3: throw ParseUtility.GenerateMissingPropertyException(objName, "time in force");
                    case 4: throw ParseUtility.GenerateMissingPropertyException(objName, "order type");
                    case 5: throw ParseUtility.GenerateMissingPropertyException(objName, "side");
                    default: throw ParseUtility.GenerateMissingPropertyException(objName, $"unknown ({missingPropNum})");
                }
            }
